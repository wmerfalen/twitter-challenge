...
const { Likes } = require("../storage/likes");
...
function FloatingProfile({ left, top, tweet }) {
  ...
  return (
    <span
      ...
      style={floatingStyle}
      onMouseOver={mouseOverFloatingDiv}
      onMouseLeave={mouseLeftFloatingDiv}
    >
...
function spawnReplyBox(event, for_tweet) {
  ...
  let existing = document.getElementById(FLOATING_REPLY_DIV);
  if (existing) {
    existing.remove();
  }
  ...
  ReactDOM.render(
    <FloatingReply for_tweet={for_tweet} />,
    document.body.appendChild(div)
  );
}
...
function tweetFooter(tweet, floating) {
  ...
  return (
    <div className="tweet-footer">
      <div className="blue-hover" onClick={(e) => spawnReplyBox(e, tweet)}>
      ...
      >
        ...
        <div className={getClassFor(tweet)} id={`${tweet.id}-likes`}>
...
function Tweet({ tweet, floating }) {
  ...
  return (
    <div className="tweet-container">
      <div
        className="profile-image-wrapper hover-cursor"
      ...
      <div className="tweet-content hover-cursor">
        <b
          ...
          onMouseOver={(event) =>
            profilePreview(tweet, event.pageX, event.pageY)
          }
          onMouseLeave={() => removeProfilePreview(tweet)}
        >
          {tweet.from}
        ...
        <span
          ...
          onMouseOver={() => profilePreview(tweet)}
          onMouseLeave={() => removeProfilePreview(tweet)}
        >
          {tweet.userName}
        </span>
